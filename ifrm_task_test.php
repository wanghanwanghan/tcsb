...
if($server_state!="99999")
{
	...
	$itm = "parent.document.getElementsByClassName(\"messageInfo\")[$cnt]";
    $itmInner = "$itm.childNodes[0].innerHTML";
	echo "<script>$itmInner='$server_stateValue';</script>";
	//$length = 0;
	//for ($i=0; $i<$itm.childNodes.length; $i++) {
    //if ($itm.childNodes.item($i).id == "tie") {
    //    break;
    //}
	//echo "<script>$itm.childNodes.item($i).innerHTML='$server_stateValue';</script>";
}
//如果是登记的
if($flag==0)
{
...
${"buffer" . $OTid}=implode("",file($template_shuaxin_sw_path));
${"buffer" . $OTid}=str_replace("{cfg_warmSec}",$cfg_warmSec,${"buffer" . $OTid});
$url="$web_path/ifrm_task_test.php?Tid=".$Tid.'&OTid='.$OTid.'&m5='.$m5.'&t='.$t;
${"buffer" . $OTid}=str_replace("{shuaxin}","Load('$url');",${"buffer" . $OTid});
echo ${"buffer" . $OTid};
?>